...
class VaSystem(VaFeature):
    ...
    def _va_get_full_exe_name_from_core(self, corefile, *args, **kwargs):
        ...
        elif (exefile == "chassis"):
            exefile = "/opt/varmour/bin/chassis-d"
        else:
    ...
    def va_get_debug_status(self, *args, **kwargs):
        """
        API to get status of debug switch
        param   : kwargs : dict
        example : va_get_debug_status(**kwargs)
            kwargs = {
                    'name' : name of debug switch, such as 'enable_sn_debug'
            }
        return:
            :string - 0/1 or a dict like this:
            {'enable_vadb_debug': '0', 'enable_io_debug': '0', 'enable_agent_debug': '0', 
             'enable_re_debug': '0', 'enable_konfd_debug': '0', 'enable_cn_debug': '0', 
             'enable_sn_debug': '0'}
        """
        logger.info("\nIn subroutine: " + sys._getframe().f_code.co_name)

        setup_ini_file = '/config-varmour/configuration/varmour_conf/setup.ini'
        output = self._access.va_shell('cat {}'.format(setup_ini_file), exit=False)
        output_list = va_parse_as_lines(output)

        debug_status = dict()
        for info in output_list:
            if 'enable_' in info:
                debug_info = info.split('=')
                debug_status[debug_info[0]] = debug_info[1]

        if 'name' in kwargs:
            name = kwargs.get('name')
            if name in debug_status:
                debug_status = int(debug_status[name])
            else:
                debug_status = 0

        logger.info('Debug status: {}'.format(debug_status))
        return debug_status
    ...
    def va_set_debug(self, action='enable', is_reboot=True, *args, **kwargs):
        ...
        if action.lower()  == 'enable':
            ...
            raise ValueError('Action "{}" not support, should be "disable/enable"'.format(action))
        ...
        for debug_name in set_debug_name:
            ...
            if opt is not None:
                setup_ini_file = '/config-varmour/configuration/varmour_conf/setup.ini'
                sed_output = self._access.va_shell('{} {}'.format(opt, setup_ini_file))
                is_updated = True
        ...
        for debug_name in set_debug_name:
            check_debug = int(debug_status_after.get(debug_map.get(debug_name), -1))
            if debug_value:
        ...
        if len(failure_debug_name) != 0:
            logger.error('Failed to {} debug {}'.format(action, failure_debug_name))
            return False
