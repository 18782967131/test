...
mandatory parameters!")
    ...
    def start(self, *args, **kwargs):
        ...
        sleeptime = 0.1
        max_times = 10
        if re.search(r'(\d+\.){3}', self._dest_ip) is None:
            sleeptime = 1
            time.sleep(2)

        self._stats = self.get_stats()
        while (self._stats == 'failed' or self._stats is None) and times <= max_times:
    ...
    def get_stats(self):
        ...
        awk_arg1 = str(self._outfile) + " | awk '{if ($1 > 0 && $1 == $4) print 1}'"
        awk_arg2 = str(self._outfile) + " | awk '{if ($1 > 0 && $4 == 0) print 2}'"
        awk_arg3 = str(self._outfile) + " | awk '{if ($1 > 0 && $4 < $1) print 3}'"
        awk_arg4 = str(self._outfile) + " | awk '{if ($1 == 0 && $4 == 0) print 4}'"
        shell("cat {}".format(self._outfile))
        if shell("{} 'transmitted' {}".format(cmd, awk_arg1)).strip() == "1":
            stats = 'successful'
        elif shell("{} 'transmitted' {}".format(cmd, awk_arg2)).strip() == "2":
            stats = "total_loss"
        elif shell("{} 'transmitted' {}".format(cmd, awk_arg3)).strip() == "3":
            stats = "partial_loss"
        elif shell("{} 'transmitted' {}".format(cmd, awk_arg4)).strip() == "4":
            stats = "send_failure"
