...
from vautils.orchestration.vcenter import VcenterConfig
...
class VaTopo(object):
    ...
    def __init__(self, test_param_yaml, topo_yaml, image=None,  **kwargs):
        """
        Initializes the device/pc/vcenter object and data from topo and test_param yaml file.
        all devices object can use in the script directly.  device object name is the same as
        uuid name. for example : dir_1.va_cli(). epi_1.va_cli(). pc1.exec_command,
        vcenter_1.VcenterHost.get_vms_by_hostname('10.11.120.110')
        Kwargs:
            test_param_yaml    : data of test parametes.
                                 control information and  user_params
        ...
        topo_data = load_yaml(self.topo_yaml).get('devices')
        self.swobjs = {}
        ...
        for dev_name in self.vmsobj.keys():
                ...
                #check device object if it is in topo file
                for dev_info in topo_data :
                    if dev_info.get('uniq_id') == dev_name :
                        if (dev_type == 'esxi' ) :
                            continue

                        if dev_type == 'vcenter' :
                            vcenter_obj= VcenterConfig.get_vcenter_config_util(self.vmsobj.get(dev_name))
                            builtins.__dict__['%s' % dev_name] = vcenter_obj
                        else :
    ...
    def config_links(self):
        ...
        if len(link_info) == 0 :
            logger.info("Not any link need to connection")
            return True
