...
class Subscriber(threading.Thread):
    ...
    def _send_subscription(self, url, only_new=False):
        ...
        if 'subscriptionId' not in resp_data:
            logging.error('Did not receive proper subscription response from APIC for url %s response: %s', url,
                          resp_data)
            resp = requests.Response()
            resp.status_code = 404
            resp._content = '{"error": "Could not send subscription to APIC"}'
...
class Commands(object):
    ...
    def _get(self, query_url):
        resp = self.session.get(query_url)
        return resp.json()

    def _get_object_list(self, fv_class, tenant=None):
        if tenant:
            tenant_url = '/tn-%s' % tenant
            query_url = ('/api/mo/uni%s.json?query-target=subtree&'
                         'target-subtree-class=%s' % (tenant_url, fv_class))
        else:
            query_url = '/api/node/class/%s.json' % fv_class
        data = self._get(query_url)
        obj_list = []
        for obj in data['imdata']:
            o = obj[fv_class]['attributes']
            obj_list.append(o)
        return obj_list

    def get_object_by_dn(self, dn, aci_class):
        query_url = ('/api/mo/%s.json' % (dn))
        data = self._get(query_url)
        try:
            return data['imdata'][0][aci_class]['attributes']
        except:
            logging.error('Failed to get ACI object at DN %s of class type %s' % (dn, aci_class))
            return None

    def get_epg_by_dn(self, dn):
        logging.debug('Getting EPG by dn: %s' % dn)
        return self.get_object_by_dn(dn, 'fvAEPg')

    def get_contract_by_dn(self, dn):
        logging.debug('Getting contract by dn: %s' % dn)
        return self.get_object_by_dn(dn, 'vzBrCP')

    def get_filter_by_dn(self, dn):
        logging.debug('Getting filter by dn: %s' % dn)
        return self.get_object_by_dn(dn, 'vzFilter')

    def get_tenants(self):
        fv_class = 'fvTenant'
        return self._get_object_list(fv_class)

    def get_epgs(self, tenant=None):
        fv_class = 'fvAEPg'
        return self._get_object_list(fv_class, tenant)

    def get_object_label(self, obj):
        query_url = ('/api/alias/mo/%s.json' % (obj['dn']))
        data = self._get(query_url)

        try:
            label = str(data['imdata'][0]['tagAliasInst']['attributes']['name'])
        except:
            label = None
        return label

    def get_object_tags(self, obj):
        query_url = ('/api/mo/%s.json?query-target=children&target-subtree-class=tagInst' % (obj['dn']))
        data = self._get(query_url)

        tag_list = []
        for t in data['imdata']:
            if 'tagInst' in t.keys():
                tag_list.append(t['tagInst']['attributes']['name'])
        return tag_list

    def get_epg_members(self, epg):
        query_url = ('/api/node/mo/%s.json?query-target=children&target-subtree-class=fvCEp' % (epg['dn']))
        data = self._get(query_url)
        members = []
        for m in data['imdata']:
            members.append(m['fvCEp']['attributes'])
        return members

    def get_epg_contracts(self, epg):
        query_url = (
        '/api/node/mo/%s.json?query-target=subtree&target-subtree-class=fvRsCons&target-subtree-class=fvRsConsIf,fvRsProtBy,fvRsProv,vzConsSubjLbl,vzProvSubjLbl,vzConsLbl,vzProvLbl&query-target=subtree' % (
        epg['dn']))
        data = self._get(query_url)

        contracts = {'provider': [], 'consumer': []}

        for c in data['imdata']:
            if 'fvRsProv' in c.keys():
                contracts['provider'].append(c['fvRsProv']['attributes'])
            elif 'fvRsCons' in c.keys():
                contracts['consumer'].append(c['fvRsCons']['attributes'])
        return contracts

    def get_contract_subject(self, contract):
        query_url = ('/api/node/mo/%s.json?query-target=children&target-subtree-class=vzSubj' % (contract['dn']))
        data = self._get(query_url)

        subjects = []
        for obj in data['imdata']:
            if 'vzSubj' in obj.keys():
                subjects.append(obj['vzSubj']['attributes'])
        return subjects

    def get_contract_relations(self, contract):
        query_url = (
        '/api/node/mo/%s.json?query-target=subtree&target-subtree-class=vzBrCP,vzRtOoBProv,vzRtOoBCons,vzRtCons,vzRtProv,vzRtAnyToCons,vzRtAnyToProv' % (
        contract['dn']))
        data = self._get(query_url)

        relations = {'providers': [], 'consumers': []}
        for obj in data['imdata']:
            if 'vzRtCons' in obj.keys():
                relations['consumers'].append(obj['vzRtCons']['attributes'])
            elif 'vzRtProv' in obj.keys():
                relations['providers'].append(obj['vzRtProv']['attributes'])
        return relations

    def get_contract_consumers(self, contract):
        query_url = (
        '/api/node/mo/%s.json?query-target=subtree&target-subtree-class=vzRtOoBCons,vzRtCons,vzRtAnyToCons' % (
        contract['dn']))
        data = self._get(query_url)
        ...
        for obj in data['imdata']:
            if 'vzRtCons' in obj.keys():
                epg = self.get_epg_by_dn(obj['vzRtCons']['attributes']['tDn'])
                consumers.append(epg)
                # Need to implement checks for other consumer object Types.  vzRtOoBCons,vzRtAnyToCons
        return consumers

    def get_contract_providers(self, contract):
        query_url = (
        '/api/node/mo/%s.json?query-target=subtree&target-subtree-class=vzRtOoBProv,vzRtProv,vzRtAnyToProv' % (
        contract['dn']))
        data = self._get(query_url)

        providers = []
        for obj in data['imdata']:
            if 'vzRtProv' in obj.keys():
                epg = self.get_epg_by_dn(obj['vzRtProv']['attributes']['tDn'])
                providers.append(epg)
                # Need to implement checks for other consumer object Types. vzRtOoBProv,vzRtAnyToProv
        return providers

    def get_subject_filters(self, subject):
        query_url = (
        '/api/node/mo/%s.json?query-target=children&target-subtree-class=vzRsSubjFiltAtt' % (subject['dn']))
        data = self._get(query_url)

        filters = []
        for obj in data['imdata']:
            if 'vzRsSubjFiltAtt' in obj.keys():
                fltr = self.get_filter_by_dn(obj['vzRsSubjFiltAtt']['attributes']['tDn'])
                # filters.append(obj['vzRsSubjFiltAtt']['attributes'])
                filters.append(fltr)
        return filters

    def get_filter_acls(self, filter_obj):
        query_url = ('/api/node/mo/%s.json?query-target=children&target-subtree-class=vzEntry' % (filter_obj['dn']))
        data = self._get(query_url)

        acls = []
        for obj in data['imdata']:
            if 'vzEntry' in obj.keys():
                acls.append(obj['vzEntry']['attributes'])
        return acls
